...
class JST_RR(object):
    ...
    def __init__(self, topics=1, sentilab=2, iteration=300,
        ...
        self.beta = beta
        self.gamma = 3.0 / (sentilab + .0)                   #gamma = 3.0 / S
...
def main():
    test = JST_RR(topics = 5, sentilab = 2, iteration = 300,
                    beta = 0.01, sigma = 3, ratings = [1,2,3,4,5], random_state = 123456789,
                    refresh = 50)
    ...
    for d in range(test.doc_num):
        ...
        if(test.docs_ratings[d][0] < 3 and test.pi_dl[d][0] < test.pi_dl[d][1] ):  #如果rating小于3并且 预测的情绪也是消极的
            cnt_neg += 1
        elif(test.docs_ratings[d][0] > 3 and test.pi_dl[d][0] > test.pi_dl[d][1]):
            cnt_pos += 1
