...
class ASUM_RR(object):
    ...
    def __init__(self, topics=1, sentilab=2, iteration=300,
                 beta=0.01, sigma = 3, ratings=None, random_state=123456789,
    ...
    def init_estimate(self):
        ...
        for m, doc in enumerate(self.docs):
            ...
            for s, st in enumerate(doc):
                ...
                self.SLS.append(sentence_senti)  # 记录sentence的sentiment l
                ...
                for t, word in enumerate(st):
                    ...
                    self.ndlz[m][sentence_senti][topi] += 1   #记录文档m seitment l topic z的数量+1
                    self.nlzw[sentence_senti][topi][self.word2id[word]] += 1  #记录word w关联sentiment l topic z的数量+1
                    self.nlz[sentence_senti][topi] += 1    #记录sentiment l和topic 关联的数量
    ...
    def loglikelihood(self):
        ...
        return _ASUM_RR._loglikelihood(nd, ndl, ndlz, nlzw, nlz, self.alpha, self.beta, self.gamma)


    def _sampling(self, rands):

        _ASUM_RR._sample_topics(self.nd, self.ndl, self.ndlz, self.nlzw, self.nlz, self.mdl, self.md, self.mlr, self.ml, self.alpha_lz,
                               self.alphasum_l, self.beta_lzw, self.betasum_lz, self.gamma_dl, self.gammasum_d, self.sigma, self.delta_lr,
                               self.deltasum_l, self.DS, self.WS, self.WLS, self.ZS, self.IS, self.DR, self.DRL, self.SS, self.SLS, rands)
...
def main():
    test = ASUM_RR(topics = 10, sentilab = 2, iteration = 500,
                    beta = 0.01, sigma = 1, ratings = [1,2,3,4,5], random_state = 123456789,
                    refresh = 50,high_beta=1.0)
    ...
    #test.read_corpus(sports_csv_path)
    #test.read_corpus(clothes_data_path)
    #读取情感字典
    ...
    for d in range(test.doc_num):
        ...
        if(test.pi_dl[d][0] <= test.pi_dl[d][1]):
            doc_neg += 1
        """
        if(test.docs_ratings[d][0] <= 2):
            doc_neg += 1
        if (test.docs_ratings[d][0] >= 4):
            doc_pos += 1
        if(test.docs_ratings[d][0] <= 2 and test.pi_dl[d][0] < test.pi_dl[d][1] ):  #如果rating小于3并且 预测的情绪也是消极的
            cnt_neg += 1
