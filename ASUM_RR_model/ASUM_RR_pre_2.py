""" ASUM RR Model using collapsed Gibbs sampling. """
...
class ASUM_RR(object):

    def __init__(self, topics=1, sentilab=2, iteration=300,
                 beta=0.01, sigma = 1, ratings=None, random_state=123456789,
                 refresh=50, high_beta = 0.7):
        ...
        self.alpha = 3.0 / (sentilab + .0) * (topics + .0)   #alpha = 3.0 /(S*K)
        self.beta = beta
        self.gamma = 1.0 #3.0 / (sentilab + .0)                   #gamma = 3.0 / S
    ...
    def init_model_parameters(self):
        ...
        for i in range(self.sentilab):
            for j in range(self.rating_num):
                if i==0: #pos的词
                    self.delta_lr[i][j] = self.ratings[j] * 10.0  #self.ratings[j]*10.0
                    self.deltasum_l[i] += self.delta_lr[i][j]
                else: #i==1 neg
                    self.delta_lr[i][j] = 10.0 * (6.0 - self.ratings[j]) #self.ratings[j]*10.0
                    self.deltasum_l[i] += self.delta_lr[i][j]
        ...
        for word in self.prior:
            if word in self.vocab:
                ...
                for l in range(self.sentilab):
                    ...
                    if l == label:
                        self.add_lw[l][self.word2id[word]] = self.high_beta #JST_RR中对再情感字典中的词，固定其情感
                    else:
                        self.add_lw[l][self.word2id[word]] = 1 - self.high_beta

        print("pos words num",pos_cnt)
        print("neg words num",neg_cnt)
    ...
    def init_estimate(self):
        ...
        for m, doc in enumerate(self.docs):
            ...
            #遍历每个单词
            for s, st in enumerate(doc):

                st_cnt += 1
                #对句子随机情感
                sentence_senti = (st_cnt) % self.sentilab
                sentence_topic = (st_cnt) % self.topics

                for t, word in enumerate(st):
                    cnt += 1    #记录单词数量+1
                    ...
                    self.SS.append(st_cnt-1)                  #记录这个单词所属的句子

                    self.DS.append(int(m))      #记录这个单词的所属的doc id
                    self.WS.append(int(self.word2id[word]))  #记录单词的id
                    self.WLS.append(int(sentence_senti))  #记录单词的sentiment
                    self.ZS.append(int(sentence_topic))   #记录单词的topic

                    self.nd[m] += 1             #记录文档m的单词数量+1
                    self.ndl[m][sentence_senti] += 1     #记录文档m中sentiment l的数量+1
                    self.ndlz[m][sentence_senti][sentence_topic] += 1   #记录文档m seitment l topic z的数量+1
                    self.nlzw[sentence_senti][sentence_topic][self.word2id[word]] += 1  #记录word w关联sentiment l topic z的数量+1
                    self.nlz[sentence_senti][sentence_topic] += 1    #记录sentiment l和topic 关联的数量
        ...
        self.WLS = np.array(self.WLS, dtype = np.int32)
        ...
        self.SS = np.array(self.SS, dtype = np.int32)
    ...
    def loglikelihood(self):
        ...
        return _ASUM_RR_2._loglikelihood(nd, ndl, ndlz, nlzw, nlz, self.alpha, self.beta, self.gamma)


    def _sampling(self, rands):

        _ASUM_RR_2._sample_topics(self.nd, self.ndl, self.ndlz, self.nlzw, self.nlz, self.mdl, self.md, self.mlr, self.ml, self.alpha_lz,
                               self.alphasum_l, self.beta_lzw, self.betasum_lz, self.gamma_dl, self.gammasum_d, self.sigma, self.delta_lr,
                               self.deltasum_l, self.DS, self.WS, self.WLS, self.ZS, self.IS, self.DR, self.DRL, self.SS, rands)
    ...
    def compute_perplexity(self,test_path):
        ...
        test_reviews, _ = utils.read_test_csv_data(test_path)
        test_data = utils.test_preprocessing(test_reviews)
        ...
        for doc in test_data:
            for word in doc:
                if word in self.word2id.keys():
                    ...
                    p-=np.log(np.sum(self.phi_lzw[:,:,id]))

        return np.exp(p / cnt)

def main(topics = 10, sigma = 1,high_beta=0.7, train_path = None, test_path = None):

    test = ASUM_RR(topics = topics, sentilab = 2, iteration = 500,
                    beta = 0.01,  sigma = sigma, ratings = [1,2,3,4,5], random_state = 123456789,
                    refresh = 50, high_beta=high_beta)

    test.read_corpus(train_path)
    #读取情感字典
    ...
    for l in range(test.sentilab):
        for z in range(test.topics):
            ...
            name = "sentiment:" + str(l) + "," + "topic:" + str(z)
            res_list = []
            for item in res:
                res_list.append(test.id2word[item])
                df[name] = res_list

    dataframe = pd.DataFrame(df)

    # 将DataFrame存储为csv,index表示是否显示行名，default=True
    dataframe.to_csv("sport_result.csv", index=False, sep=',')

    per = test.compute_perplexity(test_path)
    print("plexity:{}".format(per))


if __name__ == '__main__':

    #读取数据集
    clothes_data_path = './data/Clothes.csv'
    sports_csv_path = './data/Sports.csv'
    Videogame_csv_path = './data/Videogame.csv'


    neg_Videogame_csv_path = './nongeation_data/Videogame.csv'
    neg_Sports_csv_path = './nongeation_data/Sports.csv'
    neg_Clothes_csv_path = './nongeation_data/Clothes.csv'

    test_Videogame_csv_path = './test data/Videogame.csv'
    test_Sports_csv_path = './test data/Sports.csv'
    test_Clothes_csv_path = './test data/Clothes.csv'

    #beta = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
    #sigma = [1,2,3,4,5]

    main(topics=20, high_beta=1.0, sigma=1, train_path=neg_Sports_csv_path,
                                  test_path=test_Sports_csv_path)
