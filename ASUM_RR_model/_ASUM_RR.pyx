...
def _sample_topics(int[:] nd, int[:, :] ndl, int[:, :, :] ndlz, int[:, :, :] nlzw, 
                ...
                int[:] DS, int[:] WS,  int[:] LS, int[:] ZS, int[:] IS, int[:,:] DR, int[:,:] DRL, int[:] SS,int[:] SLS,
                double[:] rands) :
    ...
    cdef int s, w_i,temp_w,temp_l,temp_z
    cdef double r, sentiment_dist_cum, topic_dist_cum, r_dist_cum
    ...
    cdef double* sentiment_dist_sum = <double*> malloc( n_senti * sizeof(double))
    cdef double* topic_dist_sum = <double*> malloc(n_topics  * sizeof(double))
    cdef double* r_dist_sum = <double*> malloc(n_ratings * n_senti * sizeof(double))

    if sentiment_dist_sum is NULL:
        raise MemoryError("Could not allocate memory during sampling.")
    if topic_dist_sum is NULL:
        raise MemoryError("Could not allocate memory during sampling.")
    ...
    with nogil:
        for i in xrange(N):
            w = WS[i]  #单词w
            d = DS[i]  #对应的document id
            z = ZS[i]  #对应的topic z
            l = LS[i]  #对应的sentiment l
            s = SS[i]  #对应的sentiment id
            ...
            sentiment_dist_cum = 0  #相当于对senti的采样的概率分布
            topic_dist_cum = 0      #相当于对topic的采样的概率分布

            '''先固定一个句子的情感'''
            #如果和之前的句子是一样的,直接按照句子的情感赋值
            if s == previous_S:
                j = SLS[s]  #获得句子的情感
                '''对于每一种topic'''
            else:
                ...
                for sentiment_l in xrange(n_senti):
                    sentiment_dist_cum += (ndl[d, sentiment_l] + sigma * mdl[d, sentiment_l] + gamma_dl[d, sentiment_l]) /\
                                          (nd[d] + sigma * md[d]+ gammasum_d[d])
                    index = sentiment_l
                    sentiment_dist_sum[index] = sentiment_dist_cum

                '''随机采样一个sentiment j'''
                r = rands[i % n_rand] * sentiment_dist_cum  # dist_cum == dist_sum[-1]
                j = searchsorted(sentiment_dist_sum, n_senti, r)

                '''更新到句子情感中'''
                SLS[s] = j
                '''并且对后续同一个句子的单词的情感都重新赋值'''
                for w_i in xrange(i+1,N):
                    temp_w = WS[w_i]  # 同一个句子中的单词w
                    temp_z = ZS[w_i]  # 对应的topic z
                    temp_l = LS[w_i]  # 对应的sentiment l

                    '''是同一个句子的'''
                    if SS[w_i] == previous_S:

                        dec(ndl[d, temp_l])  # document d中sentiment l的单词数量-1
                        dec(ndlz[d, temp_l, temp_z])  # document d中sentiment l和topic z的数量-1
                        dec(nlzw[temp_l, temp_z, temp_w])  # 单词w和sentiment l, topic z的组合数量-1
                        dec(nlz[temp_l, temp_z])  # sentiment l和topic z的组合数量-1
            ...
            for k in xrange(n_topics):
                ...
                # 采样过程中和JST不同
                topic_dist_cum += (nlzw[j, k, w] + beta_lzw[j, k, w]) / (nlz[j, k] + betasum_lz[j, k]) \
                            * (ndlz[d, j, k] + alpha_lz[j, k]) / (ndl[d, j] + alphasum_l[j]) \
                            *(ndl[d, j] + sigma * mdl[d, j] + gamma_dl[d, j]) / (nd[d] + sigma * md[d]+ gammasum_d[d])
                index = k
                topic_dist_sum[index] = topic_dist_cum

            r = rands[i % n_rand] * topic_dist_cum  # dist_cum == dist_sum[-1]
            '''采样单词的topic'''
            res = searchsorted(topic_dist_sum, n_topics, r)
